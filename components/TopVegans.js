import { vegans } from '@/constants/tops'
...
const TopVegans = () => {
    ...
    const displaySpots = vegans.map((restaurant) => (
        <li className='my-4 flex flex-col justify-between h-40 rounded shadow-xl' style={{ backgroundImage: `url(${restaurant.background})`, backgroundSize: 'cover' }} key={restaurant.id}>
            ...
            <div className='flex items-center mt-2 flex-wrap justify-around'>
                ...
                {
                !restaurant.delivery && (
                    <p className=' w-[77px] font-bitter px-3 py-2 text-xs font-medium shadow-xl text-center text-white bg-[#A67DB8] rounded-lg hover:bg-[#A67DB8] focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-gray-500 dark:hover:bg-gray-700 dark:focus:ring-blue-800'>Walk-in</p>
                )
                }
            </div>
        </li>
