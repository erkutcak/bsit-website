...
const Brownie = () => {
  ...
  const displayIngredients = recipes[3].ingredients.map((ingredient, index) => (
    <div key={index}>
  ...
  const displayInstructions = recipes[3].instructions.map((instruction, index) => (
    <div key={index}>
      <ul className='mb-2 mx-4 md:list-none list-disc'>
          <li className='mb-1' key={index}>{instruction}</li>
      </ul>
    </div>
  ))
  ...
  return (
    <div className='bg-white/[.8] mx-4 my-4 px-5 py-[4px] rounded-lg'>
      <h6>{recipes[3].textcon}</h6>
      <div>
        <div className='flex flex-row items-center mt-4 mb-2 gap-2 md:justify-center'>
